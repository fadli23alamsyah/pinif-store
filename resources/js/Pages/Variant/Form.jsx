...
export default function Form(props) {
    ...
    const submit = (e) => {
        ...
        data.id 
            ? put(route('variant.update')) 
            : post(route('variant.store'));
    };
    ...
    return (
        <AuthenticatedLayout
            auth={props.auth}
            errors={props.errors}
            header={<h2 className="font-semibold text-xl text-white leading-tight">{data.id? 'Edit' : 'Tambah'} Varian</h2>}
        >
            <Head title={`${data.id? 'Edit' : 'Tambah'} Varian`} />

            <div className="py-6 px-6">
                <div className="p-4 sm:p-8 bg-white shadow rounded-lg">
                    <form onSubmit={submit}>
                        <div>
                            <InputLabel forInput="name" value="Nama Varian" />
