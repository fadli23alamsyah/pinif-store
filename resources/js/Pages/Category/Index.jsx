...
export default function Index(props) {
    ...
    const destroy = () => {
        Inertia.visit(route('category.destroy'),{
            method: 'delete',
            data: category,
            onFinish: () => setCategory({}),
        })
    }
    ...
    return (
        <AuthenticatedLayout
            auth={props.auth}
            errors={props.errors}
            header={<h2 className="font-semibold text-xl text-white leading-tight">Daftar Kategori</h2>}
        >
                            ...
                            {props.data.map((item, i) => 
                                <tr key={i}>
                                    ...
                                    <td className='flex flex-wrap gap-2'>
                                        <IconEditButton onClick={() => window.location.href = route('category.edit',item)} />
            ...
            <Modal show={showDelete} maxWidth="md" onClose={closeModalDelete}>
                <div className="p-4">
                    ...
                    <div className="mt-4">Apakah anda yakin ingin menghapus data {category.name !== undefined ? category.name : ''}</div>
